...
function myClick() {
  ...
  if (user_data.user_name) {
    main_div.innerHTML += '\t\t<h1><mark>Hello, ' + user_data.user_name + ', enjoy your colour!</mark></h1>\n';
  }
  else {
    main_div.innerHTML += '\t\t<h1><mark>You didnt enter your user_name, but still, enjoy your colour!</mark></h1>\n';
  }

  main.style.backgroundColor = user_data.color;

  main_div.innerHTML += '\t\t<h1><mark>' + (user_data.checked ? 'Agreed to tos :D' : 'You disagreed to the tos :◖') + '</mark></h1>\n';

  if (user_data.pizza)
    main_div.innerHTML += '\t\t<h1><mark>You like pizza!</mark></h1>\n';
  else
    main_div.innerHTML += '\t\t<h1><mark>You monster!</mark></h1>\n';
  if (sessions)
